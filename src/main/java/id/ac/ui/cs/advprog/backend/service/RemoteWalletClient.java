...
@Service
public class RemoteWalletClient implements WalletClient {

    private final RestTemplate restTemplate;
    private final String walletServiceBaseUrl;

    public RemoteWalletClient(
        RestTemplate restTemplate,
        @Value("${WALLET_SERVICE_BASE_URL:http://bidmart-wallet-service:8085}") String walletServiceBaseUrl
    ) {
    ...
    @Override
    public void hold(UUID userId, UUID auctionId, BigDecimal amount) {
        postInternal("/wallet/internal/hold", new WalletInternalFundsRequest(userId, auctionId, amount));
    }

    @Override
    public void release(UUID userId, UUID auctionId, BigDecimal amount) {
        postInternal("/wallet/internal/release", new WalletInternalFundsRequest(userId, auctionId, amount));
    }

    @Override
    public void capture(UUID userId, UUID auctionId, BigDecimal amount) {
